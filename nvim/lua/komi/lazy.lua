...
require("lazy").setup({
	...
	-- Rich presence plugin for discord ("Using Neovim btw" moment)
	{
		"vyfor/cord.nvim",
		build = "./build",
		event = "VeryLazy",
...

	{
		...
		config = function()
			...
			vim.keymap.set(
				...
				{ desc = "LspSaga: Go to previous [D]iagnostic message" }
			)
			vim.keymap.set(
				"n",
				"]d",
	...
	{ -- Autoformat
		...
		opts = {
			...
			format_on_save = function(bufnr)
				...
				local disable_filetypes = { c = true, cpp = true, css = true, html = true }
				return {
					timeout_ms = 500,
